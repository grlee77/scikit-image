...
from skimage.util import random_noise, rescale_as_float
...
def test_poisson():
    ...
    rng = np.random.default_rng(seed)
    expected = rng.poisson(rescale_as_float(data) * 256) / 256.
...
def test_clip_poisson():
    seed = 42
    data = camera()  # 512x512 grayscale uint8
    data_signed = rescale_as_float(data) * 2. - 1.  # Same image, on range [-1, 1]
