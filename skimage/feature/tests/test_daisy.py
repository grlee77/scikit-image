...
def test_daisy_desc_dims():
    img = rescale_as_float(data.astronaut()[:128, :128].mean(axis=2))
    rings = 2
    histograms = 4
    orientations = 3
...
def test_descs_shape():
    img = rescale_as_float(data.astronaut()[:256, :256].mean(axis=2))
    radius = 20
    step = 8
...
def test_daisy_incompatible_sigmas_and_radii():
    img = rescale_as_float(data.astronaut()[:64, :64].mean(axis=2))
    sigmas = [1, 2]
    radii = [1, 2]
    with pytest.raises(ValueError):
...
def test_daisy_visualization():
    img = rescale_as_float(data.astronaut()[:32, :32].mean(axis=2))
