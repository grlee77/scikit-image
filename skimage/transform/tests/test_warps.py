...
from skimage.transform._geometric import (AffineTransform,
                                          ProjectiveTransform,
                                          SimilarityTransform)
from skimage.util.dtype import rescale_as_float, _convert
...
def test_swirl(dtype):
    image = rescale_as_float(checkerboard()).astype(dtype, copy=False)
...
def test_warp_identity():
    img = rescale_as_float(rgb2gray(astronaut()))
