...
from ..util.dtype import rescale_as_float
...
def _compute_color_lut(bins, sigma, max_value, *, dtype=float):
    ...
    sigma : float
        ...
        the `rescale_as_float` function and thus the standard deviation is in
...
def denoise_bilateral(image, win_size=None, sigma_color=None, sigma_spatial=1,
    ...
    converted using the `rescale_as_float` function and thus the standard
    ...
    >>> astro = rescale_as_float(data.astronaut())
    ...
    image = np.atleast_3d(rescale_as_float(image))
...
def denoise_tv_bregman(image, weight=5.0, max_num_iter=100, eps=1e-3,
    ...
    ----------
    image : ndarray
        Input data to be denoised (converted using rescale_as_float`).
    weight : float
...
def _scale_sigma_and_image_consistently(image, sigma, multichannel,
    ...
    Images that are not floating point will be rescaled via ``rescale_as_float``.
...
def estimate_sigma(image, average_sigmas=False, multichannel=False, *,
    ...
    >>> img = rescale_as_float(skimage.data.camera())
    >>> sigma = 0.1
    >>> rng = np.random.default_rng()
