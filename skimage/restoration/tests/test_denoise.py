...
from skimage import color, data, rescale_as_float, restoration
...
astro = rescale_as_float(data.astronaut()[:128, :128])
astro_gray = color.rgb2gray(astro)
checkerboard_gray = rescale_as_float(data.checkerboard())
...
def test_denoise_bilateral_pad():
    ...
    correctly padded."""
    img = rescale_as_float(data.chelsea())[100:200, 100:200]
...
def test_wavelet_denoising_scaling(case, dtype, convert2ycbcr,
                                   estimate_sigma):
    """Test cases for images without prescaling via rescale_as_float."""
    rstate = np.random.default_rng(1234)
